 ...
 */
@Controller
@RequestMapping("/sgkj1")
public class Sgkj1Controller extends BaseController{
    ...
    @ResponseBody
    public Map<String, Object> getItems1(int length, int start, int draw,Integer xfid) throws Exception {
        Map<String, Object> result = new HashMap<String, Object>();
        Pagination pagination = new Pagination();
        pagination.setPageNo(start / length + 1);
        pagination.setPageSize(length);
        ...
        params.put("gsdm",gsdm);
        pagination.addParam("gsdm",gsdm);
        List<ZffsVo> list = zffsService.findByPage(pagination);
        int total = pagination.getTotalRecord();
        result.put("recordsTotal", total);
        result.put("recordsFiltered", total);
        result.put("draw", draw);
    ...
    public Map save() {
        ...
        try {
            ...
            String[] fphxzs = ((String) params.get("fphxz")).split(",",-1);//商品编码

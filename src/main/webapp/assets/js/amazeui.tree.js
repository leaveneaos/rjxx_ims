      ...
      $.each(items.data, function(index, value) {
        ...
        if (value.type === 'folder') {
          ...
          $entity.find('.' + _this.classes.branchName + ' > .' + _this.classes.label).html(value.title);
            $entity.find('.' + _this.classes.branchName + ' > .' + _this.classes.status).html(value.selectedStatus ? value.selectedStatus : '');
...
function styleNodeDeselected(self, $element, $icon) {
  ...
  // if ($element.data('type') === 'item' && $icon.hasClass(self.itemSelectedIcon)) {
    $icon.removeClass(self.itemSelectedIcon);
    if ($element.data('type') === 'item') {
        $icon.addClass(self.itemIcon);
    }
     // make bullet
  // }
}
...
function multiSelectSyncNodes(self, clicked, selected) {
  ...
  if (clicked.$element.hasClass(self.classes.selected)) {
    ...
    if (clicked.$folderIcon) {
        styleNodeDeselected(self, clicked.$element, clicked.$folderIcon);
    }
    // set event data
    ...
    styleNodeSelected(self, clicked.$element, clicked.$icon);
      if (clicked.$folderIcon) {
          styleNodeSelected(self, clicked.$element, clicked.$folderIcon);
      }
    // set event data
